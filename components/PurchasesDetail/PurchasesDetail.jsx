...
function PurchasesDetail({ item, close }) {
	...
	let titleImg =
		splitUrl[splitUrl.length - 1] !== "None" ? item.data?.product.title : "";
		...
		calificationStore = (
			<div className="purchase-evaluation-link">
				<div className="link-content">
					<Link href={`/resena_producto/${item.data.order_id}`}>
						<a>Calificar compra</a>
					</Link>
				</div>
			</div>
		);
				...
				<div className="vendor-info">
					<section className="info">
						...
						<section className="info-text">
							...
							<h4 className="sub-1">
								{item.data.seller.name + " " + item.data.seller.last_name}
							</h4>
