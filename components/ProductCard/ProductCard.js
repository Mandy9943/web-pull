...
export default class ProductCard extends Component {
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        if (!entry.target.parentElement.classList.contains("hidden")) {
          ...
          sendProductListViewed(this.props, this.props.index, "Products List");
  ...
  componentDidMount() {
    ...
    this.observer = new IntersectionObserver(this.callbackFunction, options);
    ...
    return (
      ...
      >
        ...
        <a href={handleFormatUrl(this.props.product_id, this.props.title)}>
          ...
          <h3
            className={this.props.statusProduct === 1 ? "" : "product-no-stock"}
          >
            ${" "}
            {this.props.price
