...
class Category extends Component {
  ...
  componentDidMount() {
    ...
    const equals = currentPage.filter(
      (value, index) => value === prevPage[index]
    );
    ...
    this.state.filters.forEach((value) => {
      ...
      switch (item[0]) {
        case "price":
          ...
          valueArray[0] === "Desde"
            ? (price =
                parseFloat(valueArray[1]) + "-" + parseFloat(valueArray[3]))
            : (price =
                parseFloat(valueArray[2]) + "-" + this.state.data.max_price);
          break;
        case "brand":
          brand = value.split("|")[1];
          break;
    ...
    filters.then((response) => {
      ...
      if (
        ...
        switch (this.state.treeSelectedCategory.length) {
          ...
          case 2:
            mergedCategories[
              this.state.treeSelectedCategory[0].index
