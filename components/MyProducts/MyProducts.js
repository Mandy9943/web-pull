...
export default class MyProducts extends Component {
	...
	loadData = (page) => {
		...
		getData('/getMyProducts/' + (page - 1), this.props.jwt).then((response) => {
			let tmp = [];
			for (let i = 0; i < response.data.length; i++) {
				tmp.push(false);
			}
			this.setState({ products: response.data, optionPanel: tmp });
		});
	};
	...
	componentDidMount() {
		...
		getData('/countMyProducts', this.props.jwt).then((response) => {
			this.setState({
	...
	changePState = async (id, newState, e) => {
		...
		if (error && error.status === 200) {
			getData('/getMyProducts', this.props.jwt).then((response) => {
				let tmp = [];
				for (let i = 0; i < response.data.length; i++) {
					tmp.push(false);
				}
	...
	render() {
		const contentModalNewProduct = (
			<div className="modal-logout">
				...
				<Link href={'/vender_producto'}>
					<button className="logout-button">Individual</button>
				</Link>
				<button onClick={this.toggleModalNewProduct} className="cancelar-button">
				...
				<div key={i} className="option-list">
					...
					<div className="option id">
						...
						<a
							...
							onClick={() => this.openOption(i)}
						>
							<FontAwesomeIcon icon={faEllipsisV} />
						</a>
						<section
							onMouseEnter={() => this.mEnter(i)}
							onMouseLeave={() => this.mLeave(i)}
							className={
						...
						>
							...
							<a
								...
								onClick={(e) =>
									this.changePState(product.product_id, product.status === 1 ? 0 : 1, e)
								}
							>
			...
			<div className="purchase-list" onClick={this.closeOptions}>
				...
				{productList.length == 0 ? (
					<section className="empty-text">
						...
						<p>Empieza ahora...</p>
						<Link href="#">
							<h5 className={'accent'} onClick={this.toggleModalNewProduct}>
								<a>Crear publicacion</a>
							</h5>
						</Link>
					</section>
				) : (
					<>
						<section className="list-publications">{productList}</section>
						<Pagination
				...
				{this.state.modalNewProduct ? (
					<Modal
						toggle={this.toggleModalNewProduct}
						content={contentModalNewProduct}
						button
					/>
				) : null}
			</div>
