    ...
    getFront("/getBanners/" + this.props.category.replace(/-/g, " ")).then(
      (response) => {
        if (response.data.files.length > 0) {
          this.setState({ showFilters: response.data.files });
        }
      }
    );
  }
  ...
  handlePrice = (e) => {
    ...
    if (Number.isInteger(from_price) && Number.isInteger(to_price)) {
      this.props.applyFilter(
        "price",
        "Desde " + from_price + " Hasta " + to_price
      );
    }
  };
      ...
      for (let it = 0; it < 4; it++) {
        if (
          ...
          if (renderedPrices.length < 5) {
            prices.push(
              "Desde " + (top - div * (it + 1)) + " Hasta " + (top - div * it)
            );
            renderedPrices.push(
              "Desde " +
          ...
          <div className="filter-group categories">
            <div className="filter-container">
              <div className="content-left">
                <span className="ots">
                  {this.props.search.replace(/-/g, " ").trim()}
                </span>
                {this.props.isSearch && (
                  <span className="breadcrumb">
                    {this.props.treeSelectedCategory.map((item, i) => (
                      <>
                        <span
                          key={item.key}
                          onClick={() =>
                            this.props.onSelectCategory(item, item.index)
                          }
                        >
                          {item.key}
                        </span>
                    ...
                    {this.props.treeSelectedCategory.length > 0 &&
                      this.props.search}
                  </span>
                )}
                            ...
                            return (
                              ...
                              >
                                <span
                                  className="node"
                                  onClick={() =>
                                    this.props.onSelectCategory(node1, i1)
                                  }
                                  style={
                                    node1.selected
                                      ? { color: "#d00a2d", fontWeight: "600" }
            ...
            <div style={{ paddingLeft: 4 }}>
              {prices.map((item, index) => (
                <div key={index}>
                  ...
                  >
                    <p className="item-filter-group show">
