...
export const validatePayCC = (payload) => {
    ...
    if( parseInt(payload.expiration_date.split("/")[0]) < d.getFullYear()
        ||
        (parseInt(payload.expiration_date.split("/")[0]) === d.getFullYear()
            &&
        parseInt(payload.expiration_date.split("/")[1]) < d.getMonth()+1)
    ){
