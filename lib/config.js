...
if (process.env.NODE_ENV !== "production") {
  ...
  baseUrl = "https://kiero.co";
  urlApi = "https://api.kieroapi.net";
  urlApiTest = "https://api.kieroapi.org";
  urlImages = "https://kiero.co/images/";
  //ROUTES for search
  searchBaseUrl = "https://api.kieroapi.net/search";
  // var searchBaseUrl = "http://30.30.30.110:5001/search";
  wompi_at_url =
    "https://sandbox.wompi.co/v1/merchants/pub_test_pYoEwV7Vh2UjsXGhNQ5JEYWa1LnXKj9r";
  // var searchBaseUrl = "http://[::1]:5000";
...
const getNumberWhatsapp = (category) => {
  // Arreglo de números con formato +xx xxx xxxxxx
  const numbers = [
    {
      category: "all",
      phone: ["+1 305 9286537"], //"+57 311 6661496", "+57 322 6569293, +1 323 7458884",,
    },
    {
  ...
  const numberReturned =
    numbers.filter((number) => {
      return (
        String(number.category).toLowerCase() === String(category).toLowerCase()
      );
    })[0] ?? numbers[0];

  // Obteniendo un número aleatorio
  const i = Math.floor(Math.random() * numberReturned.phone.length);
  const pretty = numberReturned.phone[i];
...
module.exports = {
  ...
  // filtersApi: "https://api.kieroapi.org/search/filters",
