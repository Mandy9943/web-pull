...
import { createTraceLead } from "./trace";
...
export const createlead = async (payload, step) => {
  let name = Cookies.get("name") || "" || payload.first_name;
  let email = Cookies.get("email") || "" || payload.email;
  let last_name = Cookies.get("last_name") || "" || payload.last_name;
  ...
  var step_value = "";
  if (step === 1) {
    step_value = "Usuario Registrado que Presionó el Botón Comprar";
  } else if (step === 2) {
    step_value =
      "Usuario Invitado (Y Usuario Registrado) que Presionó el botón de Continuar con la transacción";
  } else if (step === 3) {
    step_value = "Usuario Registrado que Accedió a los Detalles del Producto";
  }
  if (payload.rating !== undefined) {
    var rating = toString(payload.rating / 10);
  } else {
    rating = "N/A";
  }
  const data = {
    ...
    id: user_id,
    country: "CO",
    lead_type: step_value,
    category: payload.category.name,
    sub_category: payload.sub_category,
    price_product: payload.product_global_price,
    product_title: payload.product_global_title.substring(0, 250),
    product_description: payload.description,
    product_id: String(payload.product_global_id),
    // product_link: "",
    product_image: payload.images[0].url,
    product_brand: payload.brand,
    category_id: String(payload.category_id),
    product_category: String(payload.breadcum[0].name),
    product_subcategory: String(payload.sub_category),
    rating: rating,
    cost_of_goods_sold: String(payload.cost_of_goods_sold),
    asin_link: payload.asin_link,
    gross_margin: String(payload.gross_margin),
    gross_margin_percent:
      String(parseFloat(payload.gross_margin / payload.price).toFixed(2)) ===
      "NaN"
        ? payload.gross_margin_percent
        : String(parseFloat(payload.gross_margin / payload.price).toFixed(2)),
    margin_percent: String(payload.margin_percent),
    product_link: payload.product_link,
    weight: String(payload.weight),
    main_link: payload.main_link,
    subcategory_link: payload.subcategory_link ? payload.subcategory_link : "",
  };
  // console.log("lead", step, data);

  await createleadClient(data);
  const dataTRace = await createTraceLead(
    email,
    JSON.stringify(data),
    name,
    last_name,
  ...
  console.log("trace", dataTRace, "step", step_value);
};
