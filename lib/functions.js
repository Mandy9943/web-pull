...
export const handleFormatNumber = (number) =>{
    let formatNumber = number.toString().split(".")[0].replace(/(.)(?=(\d{3})+$)/g, "$1.")
    return formatNumber
}
...
export const handleFormatUrl = (idProduct, nameProduct) => {
    ...
    for (var i = 0, l = from.length; i < l; i++) {
        nameProduct = nameProduct.replace(new RegExp(from.charAt(i), 'g'), to.charAt(i));
    }

    nameProduct = nameProduct.replace(/[^a-z0-9 -]/g, '') // remove invalid chars
        .replace(/\s+/g, '-') // collapse whitespace and replace by -
        .replace(/-+/g, '-'); // collapse dashes

    let newUrl = '/detalle/' + idProduct + '_' + handleFormatName(nameProduct)
