...
import { urlApi,urlApiTest, baseUrl } from './config';
const API_HOST = urlApi;
const API_HOST_TEST = urlApiTest;

const getUrl = (endpoint) => API_HOST + endpoint;
const getUrlTest = (endpoint) => API_HOST_TEST + endpoint;
// const getUrlFront = (endpoint) => 'http://localhost:19999' + endpoint;
...
export const post_test = async (endpoint, data, jwt) => {
	const headers = jwt
		? {
			headers: {
				'Content-Type': 'application/json',
		...
		: { headers: { 'Content-Type': 'application/json' } };
	console.log(getUrlTest(endpoint))
	return axios.post(getUrlTest(endpoint), data, headers);
};

export const postWompi = async (endpoint, data) => {
	const headers = jwt
		? {
				headers: {
					'Content-Type': 'application/json',
