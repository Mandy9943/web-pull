...
import { createUser, recoverPass, socialLogin, resetPassword } from "../services/userApi";
...
export const signIn = async (email, password) => {
  ...
  // console.log("el rol es",res.data)
  ...
  setCookie("role", res.data.role_name);
...
export const social = async (name, last_name, email, red, id, token) => {
  ...
  setCookie("user_type", res.data.data.role_id);
  setCookie("role",res.data.data.role_name);
  setCookie("email", res.data.data.email);
  setCookie("name", res.data.data.name);
  setCookie("last_name", res.data.data.last_name);
  setCookie("full_name", res.data.data.name+" "+res.data.data.last_name);
...
export const signUp = async (name, last_name, email, password, password_confirmation) => {
  ...
  setCookie("success", `${name}, Tu cuenta ha sido creada, revisa tu cuenta de correo electrónico para activar la cuenta.`);
  redirect("/login");
  return null;
};
...
export const getRole = ctx => {
  return getCookie("role", ctx.req);
};
